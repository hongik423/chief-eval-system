...
const EVALUATOR_MANUAL = [
  { title: '0. TEST 문제 선정 이동(사전 안내)', content: '로그인 완료 후 상단 네비바 가운데의 [🗳️ TEST 문제 선정] 버튼을 먼저 클릭해 문제 선정 페이지로 이동합니다. 각 영역별 3문제(총 9문제)를 선택한 뒤 본 평가 화면으로 돌아와 평가를 진행하세요.' },
  ...
  { title: '3. 평가 방식', content: '평가는 3개 역량 영역(A·B·C)으로 구성되며, 총 9개 항목·100점 만점입니다. A. 커뮤니케이션(인터뷰) 역량(50점), B. 결과보기 제안능력(40점), C. 실행설계와 위험고지(10점)입니다. 가점(최대 10점)은 별도 적용되어 110점 만점으로 최종 산정됩니다. 평균 70점 이상 시 합격입니다.' },
...
const ADMIN_MANUAL = [
  ...
  { title: '9. 초기화', content: '[초기화] 버튼은 관리자 전용입니다. 클릭 시 관리자 비밀번호 재입력과 확인 문구 "초기화하라" 입력이 필요합니다. 실행 전 해당 기간 데이터가 자동으로 "보관 데이터" 탭에 아카이브됩니다. 초기화 후에도 [보관 데이터] 탭에서 과거 시점을 선택해 조회할 수 있습니다.' },
  { title: '10. 보관 데이터', content: '"보관 데이터" 탭에서 초기화 전 자동 보관된 평가 데이터를 조회합니다. 보관 시점 카드를 클릭하면 해당 시점의 응시자별 점수·평가위원 코멘트·합격여부를 확인할 수 있습니다.' },
];
